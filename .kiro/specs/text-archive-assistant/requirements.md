...
文本归档助手是一个跨端应用，旨在帮助用户整理和归档文本内容，包括日程安排和日记/备忘录。系统使用OCR技术识别文本内容，并通过AI模型自动分类和整理，将日程信息和日记内容分别归档到相应的存储位置。前端使用Vue和Tailwind CSS构建，后端使用Python，集成PaddleOCR-VL模型进行文本识别和分类。

## 术语表

- **文本归档助手 (Text Archive Assistant)**: 本系统的名称，用于整理和归档用户的文本内容
...
1. WHEN 用户通过前端客户端上传图片文件 THEN 文本归档助手 SHALL 接收该文件并将其传递给OCR引擎
2. WHEN 用户使用设备摄像头拍照 THEN 前端客户端 SHALL 捕获照片并将其传递给OCR引擎
3. WHEN 用户在前端客户端的输入框中直接输入文本 THEN 文本归档助手 SHALL 接收该文本内容
4. WHEN 用户上传的图片文件格式不被支持 THEN 文本归档助手 SHALL 拒绝该文件并向用户显示支持的格式列表
5. WHEN 用户上传的文件大小超过10MB THEN 文本归档助手 SHALL 拒绝该文件并提示用户文件过大
6. THE 文本归档助手 SHALL 支持JPG、PNG、JPEG和BMP格式的图片文件
...
5. THE 文本归档助手 SHALL 支持按日期范围筛选日程项
...
4. THE 文本归档助手 SHALL 支持用户为备忘录添加自定义标签
5. THE 文本归档助手 SHALL 支持按标签筛选备忘录
...
1. WHEN 用户在任意设备上登录 THEN 文本归档助手 SHALL 显示该用户的所有归档内容
...
5. THE 文本归档助手 SHALL 支持Web浏览器和移动设备访问
...
1. WHEN 用户在搜索框中输入关键词 THEN 文本归档助手 SHALL 在所有归档内容中搜索匹配的文本
2. WHEN 搜索结果包含多个匹配项 THEN 前端客户端 SHALL 按相关度排序显示结果
3. WHEN 用户搜索日程项 THEN 文本归档助手 SHALL 在时间、日期和事件描述字段中搜索
4. WHEN 用户搜索备忘录 THEN 文本归档助手 SHALL 在内容摘要、完整文本和标签中搜索
5. THE 文本归档助手 SHALL 支持模糊搜索和精确搜索两种模式
...
3. WHEN 数据在网络传输 THEN 文本归档助手 SHALL 使用HTTPS协议加密通信
